...
def save_analysis_cache(code: str, analysis_data: dict):
    ...
    try:
        ...
        record = {
            ...
            "volume": int(analysis_data.get("volume", 0) or 0),
            "amount": int(analysis_data.get("amount", 0) or 0),
            "market_cap": int(analysis_data.get("market_cap", 0) or 0),
...
def update_quote_in_cache(code: str, realtime: dict):
    ...
    try:
        ...
        sb.table("stock_analysis_cache").update({
            ...
            "volume": int(realtime.get("volume", 0) or 0),
            "amount": int(realtime.get("amount", 0) or 0),
            "market_cap": int(realtime.get("market_cap", 0) or 0),
            "quote_updated_at": now,
            "updated_at": now,
